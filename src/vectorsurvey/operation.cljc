(ns vectorsurvey.operation
  "SurveyOperationActor -- one vector-control/environmental-health
  survey request = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (Surveillance Advisor) is
  sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Public Health Governor (:govern) and the rollout phase
  ...
  each operation is auditable and checkpointed. A surveillance
  operator's field lifecycle is advanced by MANY ops (environmental-
  reading logging / vector-survey-log filing / service-visit
  scheduling / outbreak-signal escalation), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human survey coordinator (or public-health official for
  outbreak-signal escalations). The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [vectorsurvey.advisor :as advisor]
            [vectorsurvey.governor :as governor]
...
(defn build
  "Compiles a SurveyOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `vectorsurvey.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Surveillance Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Public Health Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-public-health-official-approval
