...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "cofog074-mutation-" suffix ".edn")))
...
(def laws-only
  ...
  (subset #{"law.infectious-diseases"}))
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-mhlw-go-jp" "host.id-info-jihs-go-jp"
            "law.infectious-diseases" "mhlw.kakukaisen" "jihs.idwr" "mhlw.vector-guideline"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.infectious-diseases" (fn [e] (dissoc e :egov/law-id)))
    ...
    :mutate (fn [data]
              (mapv (fn [e]
                      (if (and (= :page-identity (:source/verify e))
                               (not= "mhlw.kakukaisen" (:source/id e)))
...
(def network
  [{:id "nonexistent-law" :on :laws-only
    ...
    :mutate #(alter-entity % "law.infectious-diseases" (fn [e] (assoc e :egov/law-id "999ZZ9999999999")))
    :want-exit 1 :want-reason "nonexistent-law"}

   {:id "law-title-drift" :on :laws-only
    :why "a real id recorded under the wrong title"
    :mutate #(alter-entity % "law.infectious-diseases" (fn [e] (assoc e :egov/law-title "感染症法")))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "law-num-drift" :on :laws-only
    :why "a real id recorded under the wrong law number"
    :mutate #(alter-entity % "law.infectious-diseases" (fn [e] (assoc e :egov/law-num "平成十年法律第百十五号")))
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.infectious-diseases"
                                  (fn [e] (assoc e :egov/law-title "感染症法")))
                    (into UNROUTABLE-HOST))))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "page-title-drift"
    :why "a real page recorded under a title it does not carry"
    :mutate #(alter-entity % "jihs.idwr" (fn [e] (assoc e :page/title "感染症発生動向調査月報")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "mhlw.vector-guideline"
