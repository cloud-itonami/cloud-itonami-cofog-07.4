...
;;
;; THE CHALLENGE GUARD IS STILL LOAD-BEARING HERE, BY MEASUREMENT AND BY
;; ABSENCE. No host cited in this register served a bot challenge to a paced,
;; serial client when the register was built (2026-09-01) — both ministry
;; hosts answered plainly. The detector stays because absence at build time
;; is not a property of the host: a challenge is a 2xx at the requested URL
;; with an empty <title>, so if one ever appears, every layer of an ordinary
;; page check reads it as success except the title, and a verifier that stops
;; there reports title drift — "the register is wrong" — about a run that
;; never reached the authority. The challenge is therefore detected by its
;; own signature BEFORE the status branch, and reported as
...
;;
;; BOTH HOSTS ANSWER 404, NOT 403 OR 202, FOR A PATH THAT DOES NOT EXIST.
;; www.mhlw.go.jp serves a real 42,503-byte 「お探しのページが見つかりません。｜
;; 厚生労働省」 page at the requested URL -- status AND title both discriminate.
;; id-info.jihs.go.jp serves a bare 315-byte Apache "404 Not Found" -- no site
;; chrome at all. One more host behaviour is load-bearing and is therefore
;; written down in the register: id-info.jihs.go.jp does NOT redirect http://
;; to https:// (the http form of a real page answers 404 at port 80), while
;; www.mhlw.go.jp does. The redirect self-test must therefore aim at the mhlw
;; page, and the run below picks its probe accordingly.
;;
...
;;
;; THE REDIRECT SELF-TEST AIMS AT THE mhlw PAGE ON PURPOSE (the pool's entry
;; 1: the pool is built host-sorted, so id-info.jihs.go.jp — alphabetically
;; first — lands at 0 and www.mhlw.go.jp at 1). Measured 2026-09-01:
;; id-info.jihs.go.jp does not redirect http to https -- the http form of a
;; real page answers 404 -- so driving the redirect branch with it would
;; produce :not-2xx and read as a broken verifier when the verifier is right.
;;
